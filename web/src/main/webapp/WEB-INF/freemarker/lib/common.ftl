...
<head>
    ...
    <title><#if projectName?? && projectName?trim != "" >${projectName!}
        &nbsp;-&nbsp;<#elseif PROJECT_NAME?? && PROJECT_NAME?trim != "" >${PROJECT_NAME!}
        &nbsp;-&nbsp;</#if>${title}</title>
    ...
    <link rel="stylesheet" type="text/css"
          href="<@tags.spring.url value='/script/jquery/plug-in/showloading/showLoading.min.css' />"/>
    <link rel="stylesheet" type="text/css"
          href="<@tags.spring.url value="/script/jquery/plug-in/artDialog/skins/aero.css" />"/>
    <link rel="stylesheet" type="text/css" href="<@tags.spring.url value='/script/css/matrix-style.css'/>"/>

    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value="/script/plugins/JSON-js-master/json2.js"/>"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/core/1.11.1/jquery-1.11.1.min.js' />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value="/script/jquery/plug-in/artDialog/artDialog.min.js" />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/jquery-migrate-1.1.1.js' />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/showloading/jquery.showLoading.min.js' />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/plugins/bootstrap/bootstrap.min.js' />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/jquery.uniform.js'/>"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/plugins/select2.min.js'/>"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/jquery.dataTables.min.js'/>"></script>
    <script type="text/javascript" charset="UTF-8" src="<@tags.spring.url value='/script/web/matrix.js'/>"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/web/matrix.tables.js'/>"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/validate/core/jquery.validate.min.js' />"></script>
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/jquery/plug-in/validate/localization/messages_zh.js' />"></script>
    ...
    <script type="text/javascript" charset="UTF-8"
            src="<@tags.spring.url value='/script/myJS/jquery.validate.plugin.xinzhe.js' />"></script>
</head>
<body>
<div id="header">
    <h1 style="width:320px;">
        <a href="<@tags.spring.url value='/smkj/index' />" onclick="javascript:$('body').showLoading();"
           style="color:#FFFFFF;font-size: 30px;font-weight: bold;">Demo</a>
    </h1>
</div>
    ...
    <ul class="nav">
        ...
        <li class="dropdown <#if selected == "PERSONAL_DATA" || selected == "MODIFY_PASSWORD">cur</#if>"
            ...
            <a href="javascript:;" data-toggle="dropdown" data-target="#profile-messages" class="dropdown-toggle">
                <i class="icon-user"></i>
                <span class="text">
                    <#if DAASBANK_DSTORE_USER??>${DAASBANK_DSTORE_USER.name!}<#else><span
                            style="color : #BFBFBF; font-weight: 900; font-size: 14px;">未登录</span></#if>
                            </span>
                <b class="caret"></b>
            </a>
...
<div class="st_tree" style="overflow-y:auto;">
    <ul class="site-stats">
        ...
        <li <#if selected == "JSXWFX">class="active"</#if>>
            <a href="<@tags.spring.url value='/es/dba/皖A-50D90' />"
               onclick="javascript:$('body').showLoading();">驾驶行为分析</a>
        </li>
